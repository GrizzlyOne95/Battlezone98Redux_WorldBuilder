...
class AutoPainter:
    ...
    def generate_mat(height_data, rules, progress_callback=None):
        ...
        for rule in rules:
            ...
            # Apply Image Mask (World Machine style)
            if rule.get('mask_path') and os.path.exists(rule['mask_path']):
                try:
                    mask_img = Image.open(rule['mask_path']).convert("L")
                    # Resize to match heightmap if needed
                    if mask_img.size != (w, h):
                        mask_img = mask_img.resize((w, h), Image.Resampling.BILINEAR)
                    mask_arr = np.array(mask_img)
                    mask &= (mask_arr > 127) # Threshold at 50%
